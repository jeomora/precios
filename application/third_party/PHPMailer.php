...
class PHPMailer {
   ...
   * @var string
   */
  public $From              = 'misvales@aztecabodeguita.com';

  /**
   * Sets the From name of the message.
   * @var string
   */
  public $FromName          = 'COMPRAS BODEGUITA';
  ...
  */
   ...
   * @var string
   */
  public $Username      = 'misvales@aztecabodeguita.com';

  /**
   * Sets SMTP password.
   * @var string
   */
  public $Password      = '$AbbeyRoad01';
 ...
 /**
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    ...
    $name = trim(preg_replace_callback('/[\r\n]+/', function(){}, $name)); //Strip breaks and trim
 ...
 */
  ...
  */
  public function EncodeQP($string, $line_max = 76, $space_conv = false) {
    ...
    $out = preg_replace_callback('/^\./m', '=2E', $out); //Encode . if it is first char on a line, workaround for bug in Exchange
    fclose($fp);
    return $out;
  }
  ...
  public function MsgHTML($message, $basedir = '') {
    ...
    $this->Body = $message;
    if (empty($this->AltBody)) {
        $textMsg = trim(strip_tags(preg_replace_callback('/<(head|title|style|script)[^>]*>.*?<\/\\1>/s', function(){}, $message)));
        if (!empty($textMsg)) {
  ...
  */
  ...
  public function DKIM_HeaderC($s) {
    ...
    foreach ($lines as $key => $line) {
      ...
      $value = preg_replace_callback("/\s+/", " ", $value) ; // Compress useless spaces
