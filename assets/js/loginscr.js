				...
				pagination: {
					items: {
						default: {
							...
							more: 'Más páginas',
							input: 'Número página',
							select: 'Seleccione el tamaño de página',
							all: 'Todos',
						},
...
function updatePass(formData) {
    return $.ajax({
        url: site_url+"Usuarios/update_pass",
        type: "POST",
        cache: false,
        contentType: false,
        processData:false,
...
function isEmail(email,warn,flag){
...
function isLength(pass,warn,flag){
...
function setZeros(dude){
    var regresa = "0"+dude;
    var longi = regresa.length;
    for (var i = 0;i <= (5-longi);i++) {
        regresa = "0"+regresa;
    }
    regresa = "NC"+regresa;
    return regresa;
...
var KTFormRepeaterCompra = function() {

    var demo2 = function() {
        $('#kt_repeater_compra').repeater({
            initEmpty: false,
...
var KTBootstrapDatepickerCompra = function () {
    ...
    var demos = function () {
         // inline picker
        $('#kt_datepicker_1_compra').datepicker({
            rtl: KTUtil.isRTL(),
            todayHighlight: true,
...
function setFormatoCompra(response){
    $.each(response.compra,function(index,value){
        value.costo = value.costo === null ? 0 :value.costo;
        $(".repeaterscompra").append(
            '<div data-repeater-item="" class="form-group row align-items-center repitemestacompra" style="margin-bottom:0 !important">'+
            '<div class="col-md-5"><div class="form-group"><label>Cantidad</label><div class="input-group">'+
            '<div class="input-group-prepend"><span class="input-group-text"><i class="flaticon2-information"></i></span></div><input type="text" class="form-control id_producto" '+
            'aria-label="Producto" value="'+value.producto+'" data-id-prod="'+value.id_producto+'" id="id_prodcompra" name="id_producto" disabled></div></div>'+
            '</div><div class="col-md-3"><div class="form-group "><label>Costo</label><div class="input-group"><div class="input-group-prepend">'+
            '<span class="input-group-text">$</span></div><input type="text" class="form-control costocompra" aria-label="Costo" id="costocompra" name="costo" value="'+value.costo+'" disabled>'+
            '</div></div></div><div class="col-md-3"><div class="form-group"><label>Cantidad</label><div class="input-group"><div class="input-group-prepend">'+
            '<span class="input-group-text">#</span></div><input type="text" class="form-control cantidadcompra" aria-label="Cantidad" id="cantidadcompra"'+
            ' name="cantidad" value="'+value.cantidad+'" disabled></div></div></div><div class="col-md-1"><a href="javascript:;" data-repeater-delete="" class="btn btn-danger">'+
            '<i class="la la-trash-o"></i></a></div></div>'
        )
    });
}

$(document).off("click", ".save_compra").on("click", ".save_compra", function(event) {
    event.preventDefault();
    var flag = 1;
    var acumulador = [];

    jQuery(".repitemestacompra").each(function(ix,ex){
        var vals = $(this);
        var id_producto = vals.find("#id_prodcompra");
        if(id_producto.data("idProd") === ""){
            id_producto.css("border-color","red");
            flag = 0;
        }else{
            id_producto.css("border-color","#E4E6EF");
        }
        var cantidad = vals.find("#cantidadcompra");
        var costo = vals.find("#costocompra");
        var warn = vals.find(".cantisValidcompra");
        flag = valis(cantidad,warn,flag);
        var proveedor = $("#id_provcompra");
        acumulador.push({"id_producto":id_producto.data("idProd"),"costo":costo.val(),"cantidad":cantidad.val(),"fecha_plazo":$("#kt_datepicker_1_compra").val(),"id_proveedor":proveedor.val()})
    })
    
    if(flag){
        sendCompra(JSON.stringify({"acumulador":acumulador})).done(function(resp){
            window.open(site_url+"Compras/valecompra/"+resp, "COMPRA REGISTRADA", "resizable=no, toolbar=no, scrollbars=no, menubar=no, status=no, directories=no, width=800, height=800, left=0, top=0");
            location.reload();
        })
    }
...
function sendCompra(values){
    return $.ajax({
        url: site_url+"Compras/new_compra",
        type: "POST",
        dataType: "JSON",
        data:{
            value : values
